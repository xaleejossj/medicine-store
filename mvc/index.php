<?php
include('modelo/con_db.php');
include("../mvc/vista/template/header_sl.php");
$query_total_products = "SELECT COUNT(*) AS total_products FROM producto";
$stmt_total_products = $conn->prepare($query_total_products);
$stmt_total_products->execute();
$stmt_total_products->bind_result($total_products);
$stmt_total_products->store_result();
$stmt_total_products->fetch();

// Query to get product details
$query_products = "SELECT ID_PRODUCTO, NOMBRE_PRODUCTO, DESCRIPCION, PRECIO, CANTIDAD, IMAGEN FROM producto";
$stmt_products = $conn->prepare($query_products);
$stmt_products->execute();
$result = $stmt_products->get_result(); // Get the result set
$products = $result; // Assign result set to $products variable

?>
<!DOCTYPE html>
<html lang="es">

<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">
    <link rel="stylesheet" href="../mvc/vista/css/inicio.css">
    <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
    <title>Inicio | Medicine Store</title>

</head>

<body>


    <section class="banner">
        <img src="../mvc/vista/img/capsula1.jpg">
    </section>

    <section id="Categorias" class="icons">
        <section class="container container-features">
            <div class="card-feature">
            <a href="../mvc/vista/medicamentos.php"><i class='bx bxs-capsule' ></i> </a>
                <div class="feature-content">
                    <span>Medicamentos</span>
                </div>
            </div>
            <div class="card-feature">
            <a href="../mvc/vista/dermocosmetica.php"><i class='bx bxs-camera-plus'></i></a>
                <div class="feature-content">
                    <span>Dermocosmetica</span>
                </div>
            </div>
            <div class="card-feature">
                <a href="../mvc/vista/cuidadopersonal.php"><i class='bx bxs-home-heart'></i></a>
                <div class="feature-content">
                    <span>Cuidado Personal</span>
                </div>
            </div>
            <div class="card-feature">
                <a href="../mvc/vista/saludsexual.php"><i class='bx bx-male-female'></i></a>
                <div class="feature-content">
                    <span>Salud Sexual</span>
                </div>
            </div>
            <div class="card-feature">
                <a href="../mvc/vista/maternidad.php"><i class='bx bxs-baby-carriage'></i></a>
                <div class="feature-content">
                    <span>Bebé y Maternidad</span>

                </div>
            </div>
            <div class="card-feature">
                <a href="../mvc/vista/bienestarynutricion.php"><i class='bx bxs-heart'></i></a>
                <div class="feature-content">
                    <span>Bienestar y Nutrición</span>
                </div>
            </div>
            <div class="card-feature">
                <a href="../mvc/vista/alimentosybebidas.php"><i class='bx bxs-bowl-hot'></i></a>
                <div class="feature-content">
                    <span>Alimentos y Bebidas</span>

                </div>
            </div>

        </section>
    </section>
    <section id="producto1" class="section-p1">
    <h2>Productos disponibles</h2>
    <div class="pro-container">
    <?php while ($row = $result->fetch_assoc()) { 
        // Convertir los datos binarios de la imagen a base64
        $imagen_base64 = !empty($row['IMAGEN']) ? base64_encode($row['IMAGEN']) : null;
        $imagen_src = !empty($imagen_base64) ? 'data:image/jpeg;base64,' . $imagen_base64 : 'No disponible';
        if ($contador == 4) {
            break; // Si el contador llega a 4, se rompe el ciclo
        }
    ?>
        <div class="pro">
             <div class="imagen-producto">
            <?php if ($imagen_src !== 'No disponible'): ?>
                <img src="<?php echo $imagen_src; ?>" alt="<?php echo htmlspecialchars($producto['NOMBRE_PRODUCTO']); ?>">
            <?php else: ?>
                <p>No disponible</p>
            <?php endif; ?>
        </div>
            <div class="des">
                <a href="../mvc/vista/producto.php?id=<?php echo $row['ID_PRODUCTO']; ?>"><h5><?php echo htmlspecialchars($row['NOMBRE_PRODUCTO']); ?></h5></a>
                <div class="star">
                    <i class='bx bxs-star'></i>
                    <i class='bx bxs-star'></i>
                    <i class='bx bxs-star'></i>
                    <i class='bx bxs-star'></i>
                    <i class='bx bxs-star'></i>                                                                                            
                </div>
                <h4>$<?php echo number_format($row['PRECIO'], 2); ?></h4>
            </div>
            <button class="i" onclick="agregarC('<?php echo $row['ID_PRODUCTO']?>','<?php echo htmlspecialchars($row['NOMBRE_PRODUCTO']); ?>', <?php echo $row['PRECIO']; ?>, '<?php echo $imagen_src; ?>')">
                <div class="car">
                    <h5>Agregar al carrito <i class='bx bxs-cart-add'></i></h5>
                </div>
            </button>
        </div>
        <?php 
        $contador++; // Incrementar el contador en cada iteración
    } 
    ?>
    </div>
</section>

    <section class="barra">
        <h1> Marcas Destacadas</h1>
    </section>
    <section class="marca">
        <section class="marcas">
            <div class="laboratorio">
                <img src="../mvc/vista/img/bayer.png">
            </div>
            <div class="laboratorio">
                <img src="../mvc/vista/img/tecnoquimicas.png">
            </div>
            <div class="laboratorio">
                <img src="../mvc/vista/img/aboot.jfif">
            </div>
            <div class="laboratorio">
                <img src="../mvc/vista/img/genfar.png">
            </div>
            <div class="laboratorio">
                <img src="../mvc/vista/img/sanofi.png">
            </div>
            <div class="laboratorio">
                <img src="../mvc/vista/img/ringer.png">
            </div>
            <div class="laboratorio">
                <img src="../mvc/vista/img/pfizer.png">
            </div>
        </section>
    </section>

    <section id="producto1" class="section-p1">
    <h2>Productos disponibles</h2>
    <div class="pro-container">
    <?php 
        $contador = 5;
        while ($row = $result->fetch_assoc()) {         
        // Convertir los datos binarios de la imagen a base64
        $imagen_base64 = !empty($row['IMAGEN']) ? base64_encode($row['IMAGEN']) : null;
        $imagen_src = !empty($imagen_base64) ? 'data:image/jpeg;base64,' . $imagen_base64 : 'No disponible';
        if ($contador == 9) {
            break; // Si el contador llega a 4, se rompe el ciclo
        }
    ?>
        <div class="pro">
             <div class="imagen-producto">
            <?php if ($imagen_src !== 'No disponible'): ?>
                <img src="<?php echo $imagen_src; ?>" alt="<?php echo htmlspecialchars($producto['NOMBRE_PRODUCTO']); ?>">
            <?php else: ?>
                <p>No disponible</p>
            <?php endif; ?>
        </div>
            <div class="des">
                <a href="../mvc/vista/producto.php?id=<?php echo $row['ID_PRODUCTO']; ?>"><h5><?php echo htmlspecialchars($row['NOMBRE_PRODUCTO']); ?></h5></a>
                <div class="star">
                    <i class='bx bxs-star'></i>
                    <i class='bx bxs-star'></i>
                    <i class='bx bxs-star'></i>
                    <i class='bx bxs-star'></i>
                    <i class='bx bxs-star'></i>                                                                                            
                </div>
                <h4>$<?php echo number_format($row['PRECIO'], 2); ?></h4>
            </div>
            <button class="i" onclick="agregarC('<?php echo $row['ID_PRODUCTO']?>','<?php echo htmlspecialchars($row['NOMBRE_PRODUCTO']); ?>', <?php echo $row['PRECIO']; ?>, '<?php echo $imagen_src; ?>')">
                <div class="car">
                    <h5>Agregar al carrito <i class='bx bxs-cart-add'></i></h5>
                </div>
            </button>
        </div>
        <?php 
        $contador++; // Incrementar el contador en cada iteración
    } 
    ?>
    </div>
</section>

    
<script type="text/javascript">
        function agregarC(){
            alert("Requiere iniciar sesion")
        }
    </script>
</body>

</html>
<?php
include("../mvc/vista/template/footer.php");
?>