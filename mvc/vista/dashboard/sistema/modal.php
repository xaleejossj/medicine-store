...
// Info factura
  ...
  if ($_POST['action'] == 'anularFactura') {
    ...
    if ($_POST['action'] == 'changePasword') {
      if (!empty($_POST['CONTRASEÑA']) && !empty($_POST['passNuevo'])) {
        ...
        $msg = '';
        $arrayData = array();
        $query_user = mysqli_query($conexion, "SELECT * FROM usuario WHERE CONTRASEÑA = '$contrasena' AND ID_USUARIO = $idUser");
        $result = mysqli_num_rows($query_user);
        if ($result > 0) {
          $query_update = mysqli_query($conexion, "UPDATE usuario SET CONTRASEÑA = '$newPass' where ID_USUARIO = $idUser");
          mysqli_close($conexion);
          if ($query_update) {
            $code = '00';
